...
- `context/cli/service-lifecycle.md` (current compile-safe `ServiceLifecycle` seam in `cli/src/services/lifecycle.rs`, including default no-op diagnose/fix/setup methods against `AppContext`, doctor result/taxonomy reuse, setup outcome bridging, the shared lifecycle provider catalog/factory, hook/config/local_db lifecycle providers, implemented doctor aggregation over diagnose/fix providers, and implemented setup aggregation over `setup` providers in order config → local_db → hooks when requested)
