...
- `context/sce/cli-observability-contract.md` (implemented config-backed runtime observability contract for the flat logging + nested `otel` config-file shape with env-over-config fallback, concrete logger behavior plus the logger trait/no-op test boundary, operator-facing `sce config show` observability reporting, and the trimmed `sce config validate` status-only validation surface)
