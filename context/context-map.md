...
- `context/sce/agent-trace-db.md` (implemented `cli/src/services/agent_trace_db/mod.rs` Agent Trace database wrapper with canonical `<state_root>/sce/agent-trace.db` path, embedded `diff_traces` migration, `DiffTraceInsert<'a>`, parameterized `insert_diff_trace()`, registered setup/doctor lifecycle provider, and active `sce hooks diff-trace` dual-write persistence)
