...
- `context/sce/shared-turso-db.md` (current shared `cli/src/services/db/mod.rs` Turso database infrastructure seam, including `DbSpec`, generic `TursoDb<M: DbSpec>`, sync `execute`/`query` wrappers, generic embedded migration execution, and current concrete wrappers for `LocalDb` plus `AgentTraceDb`)
- `context/sce/agent-trace-db.md` (implemented `cli/src/services/agent_trace_db/mod.rs` Agent Trace database wrapper with canonical `<state_root>/sce/agent-trace.db` path, ordered `diff_traces` plus `post_commit_patch_intersections` migrations, typed parameterized insert helpers for diff traces and post-commit intersection rows, registered setup/doctor lifecycle provider, and active `sce hooks diff-trace` dual-write persistence)
